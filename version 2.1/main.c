...
int startgame() {
	...
	if(load_game == 1) {
		...
		setmonsterpos(&m3, row / 2, column - 2);
...
int main(int argc, char *argv[]) {
	...
	if(startgame() == WIN) {					//moving the pacman
		...
		clear();
		mvprintw(row / 4 + 5, column / 2 - 8, "Press r to return to menu.");
	...
	while(1) {
		char t = getch();
		if(t == 'r') {		//restarting the game and resetting all the variables used in the game
			clear();
			choice = 1;
			score = 0;
			points = 0;
